...
public class DeviceConnFactoryManager {
    ...
    public static final String ACTION_PRINTER_STATUS = "printer_status";
    ...
    public static final String PRINTER_STATUS_MSG = "msg";
    ...
    public static int mConnectState = CONN_STATE_DISCONNECT;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case Constant.abnormal_Disconnection://异常断开连接
                    Utils.toast(BaseApplication.getContext(), BaseApplication.getContext().getString(R.string.str_disconnect));
                    break;
                case DEFAUIT_COMMAND://默认模式
//                    Utils.toast(BaseApplication.getContext(),BaseApplication.getContext().getString(R.string.default_mode));
                    break;
                case READ_DATA:
                    ...
                    String statusStr = BaseApplication.getContext().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            currentPrinterCommand = PrinterCommand.ESC;
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            Utils.toast(BaseApplication.getContext(), BaseApplication.getContext().getString(R.string.str_escmode));
                        } else {//查询打印机状态
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    statusStr += " " + BaseApplication.getContext().getString(R.string.str_printer_out_of_paper);
                                    sendPrintStatusBroadcast(JsCode.PRINT_PAPER_ERR.getCode());
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    statusStr += " " + BaseApplication.getContext().getString(R.string.str_printer_open_cover);
                                    sendPrintStatusBroadcast(JsCode.PRINT_COVER_OPEN.getCode());
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    statusStr += " " + BaseApplication.getContext().getString(R.string.str_printer_error);
                                    sendPrintStatusBroadcast(JsCode.PRINT_ERR_OCCURS.getCode());
                                }
                                System.out.println(BaseApplication.getContext().getString(R.string.str_state) + statusStr);
                                String mode = BaseApplication.getContext().getString(R.string.str_printer_printmode_esc);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            Utils.toast(BaseApplication.getContext(), BaseApplication.getContext().getString(R.string.str_tscmode));
                        } else {
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(BaseApplication.getContext().getString(R.string.str_state) + statusStr);
                                String mode = BaseApplication.getContext().getString(R.string.str_printer_printmode_tsc);
                                Utils.toast(BaseApplication.getContext(), mode + " " + statusStr);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                                BaseApplication.getContext().sendBroadcast(intent);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcast(CONN_STATE_CONNECTED);
                            Utils.toast(BaseApplication.getContext(), BaseApplication.getContext().getString(R.string.str_cpclmode));
                        } else {
                            if (cnt == 1) {
                                System.out.println(BaseApplication.getContext().getString(R.string.str_state) + statusStr);
                                ...
                                String mode = BaseApplication.getContext().getString(R.string.str_printer_printmode_cpcl);
                                Utils.toast(BaseApplication.getContext(), mode + " " + statusStr);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
