...
//                        SpUtils.getInstance().clearLogin();
//                        ArmsUtils.killAll();
//                        Intent intent = new Intent(context, LoginRolesActivity.class);
//                        ArmsUtils.startActivity(intent);
                    });
//                    JsMethodApi.doJsByJson("tokenOutTime","{'status':401,'token':'token 已过期'}");
                } else if (status == 201) {
                    RxUtils.runOnMainThread(o ->  ArmsUtils.makeText(message));
                }
            } catch (Exception e) {
                e.printStackTrace();
                return response;
    ...
    public Request onHttpRequestBefore(@NonNull Interceptor.Chain chain, @NonNull Request request) {
        ...
        return chain.request().newBuilder()
                .header("token", SpUtils.getInstance(context).getToken())
