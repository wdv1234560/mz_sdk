            ...
            public LocalMedia doInBackground() {
                ...
                if (!isAudio) {
                    if (PictureMimeType.isContent(config.cameraPath)) {
                        ...
                        //TODO:是图片旋转图片
                        if(PictureMimeType.isHasImage(mimeType)&&!TextUtils.isEmpty(media.getAndroidQToPath())&&intent.getBooleanExtra(PictureConfig.EXTRA_IS_FRONT_CAMERA,false)){
                            int degree = PictureFileUtils.readPictureDegree(getContext(), media.getAndroidQToPath());
                            BitmapUtils.rotateImage(degree, media.getAndroidQToPath());
                        }
                    } else {
